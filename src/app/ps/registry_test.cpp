...
TEST_CASE("create-br-from-ba", "[reg_json]")
{
    ...
    // Create border_router with network
    {
        ...
        network nwk;
        REQUIRE(reg.get(network_id{0}, nwk) == registry_status::REG_SUCCESS);
    ...
    // Create border_router with network and domain
    {
        ...
        border_router ret_val;
        REQUIRE(reg.get(border_router_id{1}, ret_val) == registry_status::REG_SUCCESS);
        REQUIRE(ret_val.nwk_id.id == 1);
        network nwk;
        REQUIRE(reg.get(network_id{1}, nwk) == registry_status::REG_SUCCESS);
        REQUIRE(nwk.id.id == 1);
        REQUIRE(nwk.dom_id.id == 0);
        domain dom;
        REQUIRE(reg.get(domain_id{0}, dom) == registry_status::REG_SUCCESS);
    }
    ...
    // Update border_router fields
    {
        ...
        border_router val;
        REQUIRE(reg.get(border_router_id{0}, val) == registry_status::REG_SUCCESS);
        ...
        network nwk;
        REQUIRE(reg.get(val.nwk_id, nwk) == registry_status::REG_SUCCESS);
        ...
        REQUIRE(reg.get(border_router_id{0}, new_val) == registry_status::REG_SUCCESS);
